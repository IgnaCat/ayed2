...
unsigned int extra_space_fee_cost (DeliveryTable a) {
  ...
  for (unsigned int hour = 0u; hour <= FEE_CLOSE_HOUR; ++hour) {
      if (a[boxes][hour].items_amount > 0){
        total_boxes += a[boxes][hour].items_amount;
      }
      if (a[letters][hour].items_amount > 0){
        total_letters += a[letters][hour].items_amount;
      }

  }

  return BOX_PENALTY*(total_boxes - MAX_ALLOWED_BOXES) + LETTER_PENALTY*(total_letters - MAX_ALLOWED_LETTERS);
}
...
void array_from_file(DeliveryTable array, const char *filepath) {
  ...
  while (!feof(file)) {
    ...
    array[letters][flight_letters.hour-1] = flight_boxes;
